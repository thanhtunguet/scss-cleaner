...
function App() {
  ...
  const handleChangeScss = React.useCallback(
    (...[, , data]) => {
      setScss(data);
    },
    [],
  ...
  const handleChangeClasses = React.useCallback(
    (...[, , data]) => {
  ...
  const handleClean = React.useCallback(
    () => {
      const newScss: string = appService.removeUnusedMixins(appService.clean(scss, usedClasses));
      const removedLines: number = appService.getNumLineOfCodeRemoved(scss, newScss);
